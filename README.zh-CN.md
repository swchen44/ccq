...
> **支持语言:** C / C++(以及 clangd 也能解析的 Objective-C 与 CUDA)。其他语言 —— Rust、Go、Python… —— **刻意不在范围内**;它们有各自的 language server(rust-analyzer、gopls…)。ccq 是 clangd 专精工具。
...
ccq export --format html --focus lookupCommand --out graph.html   # 交互知识图（离线、零依赖）
```
...
**与 CodeGraph 的关系。** ccq 是*受 CodeGraph 启发*，并**移植了它的 `c-fnptr-synthesizer.ts`** 启发式 —— 但跑在 clangd 的编译器级引擎上（而非 tree-sitter），且维持零依赖 Go binary（无 Node bundle）。为了让熟 CodeGraph 的人无痛上手，`ccq mcp` 提供 **CodeGraph 兼容**的 MCP 接口，以 `explore` 为头牌工具。我们刻意**没有** fork CodeGraph（它是 MIT，我们可以）—— 把 clangd 后端加进它 5,689 行的 tree-sitter extractor 是「换引擎」，终点只是一个更重的 ccq；见 [docs/benchmark.md §6](docs/benchmark.md)。
...
一场真实的 Claude Code A/B —— **同一 model、同一 prompt，唯一差别是 `ccq` 有没有在 `$PATH` 上** —— 直接从每次运行的 token/成本 JSON 测量（[token-cost case study](docs/case-studies/token-cost/README.md)）：

| | 用 ccq vs grep+read |
|---|---|
| **每任务 token** | **少 1.8–7.9×** |
| **每任务成本** | **省 2.1–12.4×**（全套 6.7×） |
| **速度** | ~**快 6×**（例：162s → 13s） |
| **可预测性** | 同样的事跑三次,没 ccq 成本最多差 **15×**;有 ccq 几乎**持平** |
| **完成度** | 一个 **no-build** 函数指针任务，没 ccq 得 **0%、有 ccq 得 100%** |

也就是说：用 `ccq` 驱动 C/C++ 的 agent，每个问题便宜数倍、快数倍 —— 而在 no-build 代码（驱动、ops 表、callback）的函数指针分派上，它能答出 grep+read agent **静默答错**的问题。相对地，ccq 只是一个零依赖 Go binary 要维护；即使用量打到 1/10，仍回本数倍。完整 ROI 模型与诚实 caveat 见 [case study](docs/case-studies/token-cost/README.md)。
...
**导出（用你自己的工具查）**
- `export [--format json|sql|html] [--focus <sym> [-d N]] [--out f]` — 导出调用图（含 fnptr 边）。`--format sql | sqlite3 g.db` 用纯 SQL 查（零依赖、取代 Cypher）；**`--format html` 输出自包式、离线的交互知识图**（像 CodeGraph 那种,但由 clangd 驱动）；`--focus <sym>` 只建邻域（大 repo 快,html 推荐搭配）。
- `fnptr` — 验证 fn-pointer 对照表（`ccq.fnptr.json`）

**Serve（CodeGraph 兼容）**
- `mcp` — 用 **Model Context Protocol**（JSON-RPC/stdio）服务 ccq，零额外依赖。提供 `explore`（头牌）、`callers`、`callees`、`def`、`refs`、`search`、`impact`、`symbols`、`macro` —— 让 MCP 客户端、以及熟 CodeGraph 的人零学习成本驱动 ccq。

**项目**
- `init` — 检测/生成 `compile_commands.json`（CMake / Meson / bear），没有 build 系统时则生成**无 build 的 `compile_flags.txt`**；预热 clangd
- `--compdb a.json,b.json` — 指定一个或多个**任意文件名**的 compile database（多 target build 会产生多份、常被改名）。会自动合并;每个不同的组合各自获得**独立常驻 clangd**（一个 build config 一个 daemon —— 切 config 不重索引）。见 [docs/design.md §6](docs/design.md)。
- `--config <p>` / `ccq.json` — 配置文件(`./ccq.json` > `~/.config/ccq/ccq.json` > `--config`),用 **`allow`/`deny` regex** 控制哪些文件要索引;`ccq config` 显示生效配置。
- `wait-index` — **阻塞到索引完成才返回**(agent 先跑这个,避免在索引途中查到不完整结果);`--background` 立即返回、`--rebuild` 强制重建索引。`status` 报告 `ready`/`indexing…`/`not running`。
- `cache [list|clean|path]` — 观察/清理索引缓存(daemon state、staged DB、clangd 的 `.cache/clangd`);显示大小/日期/项目。`clean --older-than N | --project p | --all [--index]`。
- `doctor` — dump 版本、配置文件、compile DB 模式、cache 大小与 daemon 状态,帮 debug。
- `status`、`shutdown`、`version`

**差异化**
- **fn-pointer 分派** — `callers`/`explore` 解析 ops-struct 注册与 `obj->fn()` 分派，合成 `dispatcher → handler` 边。以 **(struct 类型, 字段)** 为键，不同 struct 同名字段不互相污染；处理 **designated init、positional table `{"n", fn}`、field←field 传递**（移植自 CodeGraph 的合成器），另含 **`union` 函数指针字段、嵌套 struct 初始化 `.a = { .b = h }`、数组索引 designator `[N] = {…}`、pointer typedef receiver、以及 deref／cast／跨行分派（`(*pp)->f()`、`((struct T*)v)->f()`、`p->`⏎`f()`）**。clangd 自己不做这个。
...
- **无 build 模式** — 当没有 `compile_commands.json` 也没有 build 系统时，`ccq init` 会写一个 `compile_flags.txt`（自动探测 `-I` include 目录）。clangd 即能**跨文件**解析（配合 ccq 打开文件预热）**而不需 build** —— cbm 式广度，精度较低。被过度纳入的是**猜出来的 `-I` 集合**；`#ifdef` 则相反——因为**没有 `-D`**，clangd 以默认值评估条件，**被停用 config 包住的 code 会 inactive → 找不到**（不是「过度涵盖」）。ccq 的文字路径不评估 `#ifdef`，补上这个洞：fnptr 启发式仍看得到停用分支里的 dispatch，`ccq def`/`search` 也会 fallback 到纯文本[定义索引](#differentiators)（明确标示）去找 clangd 漏掉的符号。精度阶梯：compile_commands.json > compile_flags.txt > 同文件。
- **`#ifdef`-blind 定义索引(no-build fallback)** — 一个纯文本、不评估预处理器的「定义位置」索引(函数/struct/union/enum/typedef/宏),扫全部文件、不评估 `#ifdef`。`ccq def`/`search` **只在 clangd 找不到时**才查它,所以藏在**停用** config 里(no-build 下被 clangd 丢掉)的符号仍找得到 —— 明确标示为近似,绝不混进 clangd 的精确结果。
...
```
npm 只会装符合你平台的 prebuilt binary(darwin/linux x64+arm64、win32 x64)。仍需 PATH 上有 `clangd`。内网/air-gapped 请改用下面的源码或 prebuilt binary 安装(npm 需要 registry)。
...
### 安装 agent skill
把 `skills/ccq/SKILL.md` 复制到 agent 的 skills 目录：
...
| **初次索引 / build** ⚠️ | ~4s | ~11–14s | 需完整 build + 索引 | 需完整 build + 索引 | **需完整 build + ~30s 索引（ccq 最弱项）** |
| 热重复查询 | 每次重跑 | 每次重跑 | — | 每次重跑 | **~0.07–0.6s** |
| 编辑 | — | — | rename | ✅ LSP | ✅ rename / replace-body / insert |
| 集成方式 | MCP | MCP | LSP（编辑器） | MCP | **CLI + agent skill + MCP**（`ccq mcp`） |
| 安装足迹（内网） | 257 MB，vendored build | 188 MB Node bundle | ~100–350 MB binary | 🔴 ~890 包 + 下载 clangd | **单一 Go binary，0 Go 依赖**（需 clangd） |

**强项** ✅ — 唯一通过全部 8 项难 C 特性；保留 clangd 的赢面（`#ifdef`、宏、`typedef`、`_Generic`、函数级调用图）**并**加上 fn-pointer 分派（+ `ccq.fnptr.json` 对照表补盲区）；重复查询最快（warm daemon）；「智能工具」中足迹最轻（零依赖 Go binary —— 内网最佳智能选项）；符号级编辑 + graph export。

**弱项** ⚠️ — **初次索引最慢**（需真正 build 出 `compile_commands.json` + ~30s clangd 后台索引；tree-sitter 工具数秒、cscope ~0.5s 就好）；**依赖外部 clangd binary**；fn-pointer 启发式是过度近似（盲区请在 `ccq.fnptr.json` 声明）；**仅 C/C++**、**默认无 MCP**。ccq 以冷启动成本换精度，并用 warm daemon 与 `--incremental` 缓解。

完整数据 —— 特性、调用图召回、**索引速度 vs cscope/ctags/cflow**、安装与依赖足迹、使用方式、每个工具的强弱项 —— 都在 **[docs/benchmark.md](docs/benchmark.md)**。
...
- **无 build 模式精度** — `compile_flags.txt` 让你不用 build 也能跨文件，但用猜的 include 且无 `-D`：clangd 以默认值评估 `#ifdef`，**被停用 config 包住的 code 会 inactive、找不到**（clangd 端的漏看），依赖宏的代码也可能错。缓解：`ccq def`/`search` 会 fallback 到纯文本、不评估 `#ifdef` 的[定义索引](#differentiators)，仍能定位这类符号（标示为近似）。要 config 精准请用真的 `compile_commands.json`。
...
- **多份 compile DB（`--compdb`）** — clangd 对同一文件只取**一条** entry,所以当一个文件被多个 target 用不同 `-D` 编译时,**排第一的 `--compdb` 胜出**,其他 config 的 `#ifdef` 分支会 inactive。请据此排序 `--compdb`,或逐 target 各查一份 `--compdb` 取精确视图。实例:[docs/case-studies/multi-target-compdb](docs/case-studies/multi-target-compdb/README.md)。
...
> 文档：[case studies](docs/case-studies/)（实例 + 图,用看的不用听的）· [design.md](docs/design.md) · [requirement.md](docs/requirement.md) · [benchmark.md](docs/benchmark.md)
...
- 除了测试套件，[case studies](docs/case-studies/) 在真实 repo（redis、wpa_supplicant）实跑 ccq —— **抓到并修了 9 个单元测试漏掉的真 bug**（[bugs-found.md](docs/case-studies/bugs-found.md)）。写 case study 本身就是测试。
...
| 版本 | 日期 | 重点 |
|------|------|------|
| [**0.6.5**](https://github.com/swchen44/ccq/releases/tag/v0.6.5) | 2026-07-01 | `#ifdef`-blind 定义索引(no-build 下 `def`/`search` fallback);fnptr 召回 +2 写法(K&R 行首名、指令式 `ops.f = h;`)→ wpa `.scan2` 5/5;更正 no-build `#ifdef` 文档 |
...
- [x] fnptr 启发式：positional table 边界、typedef 表、嵌套 row、混用、cast/macro handler + 注释感知多行解析 — *0.6.3 完成*
- [x] fnptr 启发式：更多分派写法（union 字段、嵌套 struct 初始化、数组索引 designator、pointer typedef receiver、deref/cast/跨行分派）— *0.6.4 完成*
